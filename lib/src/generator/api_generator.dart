...
class ApiGenerator {
  ...
  /// 1. モデルクラスの生成
  /// 2. Retrofitクライアントの生成
  /// 3. APIクライアントの管理クラスの生成（ManagerGeneratorを使用）
  Future<void> generate({required String apiName}) async {
    ...
    // APIごとのディレクトリを作成
    final apiOutputDir = Directory('$outputPath/$apiName');
    if (!await apiOutputDir.exists()) {
      await apiOutputDir.create(recursive: true);
    }

    // モデルを生成
    final modelGenerator = ModelGenerator(spec, apiOutputDir.path);
    await modelGenerator.generate(apiName: apiName);

    // Retrofitのクライアントを生成
    final retrofitGenerator =
        RetrofitGenerator(spec, apiOutputDir.path, basePath: basePath);
    await retrofitGenerator.generate(apiName: apiName);

    // インデックスファイルを生成
    final indexFile = File('${apiOutputDir.path}/index.dart');
    await indexFile.writeAsString('''
// Generated by OpenAPI Generator

export 'models_index.dart';
export 'retrofit_index.dart';
''');
  }
}
